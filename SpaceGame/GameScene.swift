...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
      ...
      motionManager.startAccelerometerUpdates(to: OperationQueue.current!){ (data:CMAccelerometerData?, error:Error?) in
        if let accelerometerData = data{
          ...
          self.xAccleration = CGFloat(acceleration.x)*0.75 + self.xAccleration*0.25
  ...
  func didBegin(_ contact: SKPhysicsContact) {
    ...
    if (firstBody.categoryBitMask & photonTorpedoCategory) != 0 &&
      ...
      torpedoDidCollideWithAline(torpedoNode: firstBody.node as! SKSpriteNode, alienNode: secondBody.node as! SKSpriteNode)
    }
  }
  
  func torpedoDidCollideWithAline(torpedoNode:SKSpriteNode, alienNode:SKSpriteNode){
  ...
  override func didSimulatePhysics() {
    player.position.x += xAccleration * 50
